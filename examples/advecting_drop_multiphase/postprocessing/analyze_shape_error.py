...
def initial_condition(x, y, z, eps):
    """
    Generate the initial condition for advecting drop (2D cylindrical drop).

    Args:
        x: x-coordinates
        y: y-coordinates
        z: z-coordinates (unused, kept for interface consistency)
        eps: interface thickness parameter

    Returns:
        field: Initial condition field
    """
    # 2D drop with diameter D=1.0 (radius=0.5) centered at (1.0, 1.0) in 2x2 domain
    # Radius depends only on x and y, matching the Fortran IC in advecting_drop.f90
    center_x = 1.0
    center_y = 1.0
    radius = 0.5

    # Create the cylindrical drop with tanh profile (2D radius, no z dependence)
...
def main():
    ...
    ax.set_title(
        r"Shape Error vs $\gamma/u_{\mathrm{max}}$ (for each $\varepsilon/\Delta x$)",
        fontsize=14,
    )
    ...
    ax.set_ylabel("Shape Error (L1 norm)", fontsize=12)
    ax.set_title(
        r"Shape Error vs $\varepsilon/\Delta x$ (for each $\gamma/u_{\mathrm{max}}$)",
        fontsize=14,
    )
    ...
    if len(unique_gammas_nd_nonzero) > 1 and len(unique_epsilons_nd) > 1:
        ...
        colorbar_min = 1e-5  # Lower limit (just below data min ~1.4e-5)
        colorbar_max = 7e-2  # Upper limit (just above data max ~6.2e-2)
