...
params = params_c("insitu/inputs.json")
...
if params.decomposition != "user":
    md = md_dic[params.decomposition](params_c=params, logger = logger, comm = comm)
else:
    from md_user import USER_c 
    md = USER_c(params_c=params, logger = logger, comm = comm)
