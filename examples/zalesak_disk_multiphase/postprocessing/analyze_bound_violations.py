...
def main():
    ...
    print("=" * 80)
    print("Bound Violation Analysis for Zalesak Disk Parameter Sweep")
    print("=" * 80)
    ...
    ax.set_title(r"Maximum Scalar Field Bound Violation vs $\gamma/u_{\mathrm{max}}$ (for each $\varepsilon/\Delta x$)", fontsize=14)
    ...
    ax.set_ylabel("Maximum Bound Violation", fontsize=12)
    ax.set_title(r"Maximum Scalar Field Bound Violation vs $\varepsilon/\Delta x$ (for each $\gamma/u_{\mathrm{max}}$)", fontsize=14)
    ...
    # Plot 4: Violation fraction vs gamma/u_max
    fig, ax = plt.subplots(1, 1, figsize=(10, 6), dpi=150)
    ...
    ax.set_title(r"Percentage of Points Violating Bounds vs $\gamma/u_{\mathrm{max}}$", fontsize=14)
    ax.legend(loc="best", fontsize=9, ncol=2)
    ax.grid(True, which="both", ls="--", alpha=0.5)
    ...
    if no_violation_cases:
        ...
        for r in no_violation_cases[:5]:  # Show first 5
            print(f"  γ/u_max = {r['gamma']/u_max}, ε/Δx = {r['epsilon']/delta_x:.2f}")
