#!/usr/bin/env python3
"""
Create Animation of Zalesak Disk

This script creates an animation showing the Zalesak disk rotating
through the domain and returning to its original position.
"""
...
def create_frame_images(
    output_dir, frames_dir, mesh_size=None, gamma=None, epsilon=None
):
    """
    ...
    for i, field_file in enumerate(field_files):
        try:
            ...
            # Plot interface contour (phi=0.5)
            ax.tricontour(
            ...
            ax.set_title(
                f"Zalesak Disk - Time: {time:.3f}", fontsize=16, fontweight="bold"
            )
            ax.set_aspect("equal")
            ax.set_xlim([0, 1])
            ax.set_ylim([0, 1])
            ...
            cbar = plt.colorbar(tcf, ax=ax, label="φ", ticks=np.linspace(0, 1, 11))
            ...
            print(
                f"  Frame {i+1}/{len(field_files)}: t={time:.3f} -> {frame_file.name}"
            )
...
def create_animation(
    output_dir, animation_file, mesh_size=None, gamma=None, epsilon=None
):
    """
    ...
    Args:
        output_dir: Directory containing field files
        animation_file: Output animation filename (e.g., 'zalesak_disk.mp4')
        mesh_size: Mesh size parameter (optional)
        gamma: Gamma parameter (optional)
        epsilon: Epsilon parameter (optional)
    """
    ...
    def animate(frame_idx):
        ...
        ax.set_xlabel("x", fontsize=14)
        ax.set_ylabel("y", fontsize=14)
        ax.set_title(f"Zalesak Disk - Time: {time:.3f}", fontsize=16, fontweight="bold")
        ax.set_aspect("equal")
        ax.set_xlim([0, 1])
        ax.set_ylim([0, 1])
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create animation of Zalesak disk simulation"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    print("=" * 80)
    print("Zalesak Disk Animation Creator")
    print("=" * 80)
