...
!  STEP 1 species variant: identical physics to hotzone_trip.f90, but the
!  startup hook must not create a singular 'case.scalar' node (see below).
!
...
contains
  ...
  subroutine startup(params)
    ...
    ! NOTE: do NOT add "case.scalar.lambda"/"case.scalar.cp" here. This case
    ! uses the "scalars" ARRAY, and case.f90 tests valid_path('case.scalar')
    ! BEFORE 'case.scalars' -- so adding case.scalar.* invents a singular
    ! scalar node holding only lambda/cp, Neko takes the single-scalar branch,
    ! and dies with "Parameter solver.type missing". lambda and cp for the
    ! temperature scalar are set in the case file instead.
