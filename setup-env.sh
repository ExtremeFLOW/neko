#!/bin/bash
# Environment setup for neko-multiphase-parameter-analysis
#
# Usage:
#   source setup-env.sh --local     # macOS development
#   source setup-env.sh --cluster   # HPC cluster (Dardel/Cray + ROCm)

if [ "$1" = "--local" ]; then
    # ── macOS local development ──────────────────────────────────────────
    export JSON_INSTALL="$HOME/local"
    export NEKO_PARAMS_PREFIX="$HOME/local/neko-params"
    ...
    export PATH="$NEKO_PARAMS_PREFIX/bin:$PATH"

    echo "neko-params environment loaded (local/macOS)"
    ...
    export NEKO_PARAMS_PREFIX="$KTHMECH_PROJECT/software/neko-params"
    ...
    echo "neko-params environment loaded (cluster/Dardel)"

else
    echo "Usage: source setup-env.sh --local|--cluster"
