!> Implements the device kernel for the `most_convective_t` type.


!!!! TEMPORARY PLACE HOLDER FILE: TO DEVELOP FROM ZERO!!!!
    
module most_convective_device
...
#ifdef HAVE_HIP
  interface
     subroutine hip_most_convective_compute(u_d, v_d, w_d, temp_d, &
          ind_r_d, ind_s_d, ind_t_d, ind_e_d, &
          n_x_d, n_y_d, n_z_d, h_d, &
          tau_x_d, tau_y_d, tau_z_d, n_nodes, lx, &
          kappa, z0, tstep) &
          bind(c, name = 'hip_most_convective_compute')
     ...
     end subroutine hip_most_convective_compute
  end interface
#elif HAVE_CUDA
  interface
     subroutine cuda_most_convective_compute(u_d, v_d, w_d, temp_d, &
          ind_r_d, ind_s_d, ind_t_d, ind_e_d, &
          n_x_d, n_y_d, n_z_d, h_d, &
          tau_x_d, tau_y_d, tau_z_d, n_nodes, lx, &
          kappa, z0, tstep) &
          bind(c, name = 'cuda_most_convective_compute')
       use, intrinsic :: iso_c_binding, only : c_ptr, c_int
       use num_types, only : c_rp
       implicit none
       type(c_ptr), value :: u_d, v_d, w_d, temp_d
       ...
       type(c_ptr), value :: tau_x_d, tau_y_d, tau_z_d
       integer(c_int) :: n_nodes, lx, tstep
     end subroutine cuda_most_convective_compute
  end interface
#elif HAVE_OPENCL
#endif
  public :: most_convective_compute_device

contains
  !> Compute the wall shear stress on device using the rough log-law model.
  !! @param t The time value.
  !! @param tstep The current time-step.
  subroutine most_convective_compute_device(u_d, v_d, w_d, temp_d, &
       ind_r_d, ind_s_d, ind_t_d, ind_e_d, &
...
#if HAVE_HIP
    call hip_most_convective_compute(u_d, v_d, w_d,temp_d, &
         ind_r_d, ind_s_d, ind_t_d, ind_e_d, &
         n_x_d, n_y_d, n_z_d, h_d, &
         tau_x_d, tau_y_d, tau_z_d, n_nodes, lx, kappa, z0, tstep)
#elif HAVE_CUDA
    call cuda_most_convective_compute(u_d, v_d, w_d,temp_d, &
         ind_r_d, ind_s_d, ind_t_d, ind_e_d, &
         n_x_d, n_y_d, n_z_d, h_d, &
         tau_x_d, tau_y_d, tau_z_d, n_nodes, lx, kappa, z0, tstep)
#elif HAVE_OPENCL
  ...
  end subroutine most_convective_compute_device
end module most_convective_device
