...
#elif HAVE_CUDA
  interface
     subroutine cuda_richardson_compute(u_d, v_d, w_d, temp_d, &
          ind_r_d, ind_s_d, ind_t_d, ind_e_d, &
          n_x_d, n_y_d, n_z_d, h_d, &
...
contains
  ...
  subroutine richardson_compute_device(u_d, v_d, w_d, temp_d, &
    ...
    case ("dirichlet")
       bc_type_int = 1
    case default
       call neko_error("Neumann/Dirichlet bc not specified correctly (richardson_device)")
    end select

#if HAVE_HIP
    call hip_richardson_compute(u_d, v_d, w_d,temp_d, &
         ind_r_d, ind_s_d, ind_t_d, ind_e_d, &
         n_x_d, n_y_d, n_z_d, h_d, &
