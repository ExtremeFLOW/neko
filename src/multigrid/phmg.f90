module phmg
  ...
  type, private :: phmg_lvl_t
     ...
     type(dofmap_t) :: dm_Xh
     type(gs_t) :: gs_h
     type(cheby_t), allocatable :: cheby
...
contains
  ...
  subroutine phmg_init(this, msh, Xh, coef, dof, gs_h, bclst)
    ...
    do i = 1, this%nlvls
       ...
       allocate(this%phmg_hrchy%lvl(i)%r(this%phmg_hrchy%lvl(i)%dm_Xh%size()))
  ...
  subroutine phmg_solve(this, z, r, n)
    ...
    call this%intrp(2)%map(this%phmg_hrchy%lvl(2)%r, this%r, this%msh%nelv, &
                          this%phmg_hrchy%lvl(2)%Xh)
    call this%phmg_hrchy%lvl(2)%gs_h%op(this%phmg_hrchy%lvl(2)%r, &
                                        this%phmg_hrchy%lvl(2)%dm_Xh%size(), &
                                        GS_OP_ADD)
    call col2(this%phmg_hrchy%lvl(2)%r, this%phmg_hrchy%lvl(2)%coef%mult, &
              this%phmg_hrchy%lvl(2)%dm_Xh%size())
    !restrict residual to crs    
    call this%intrp(1)%map(this%phmg_hrchy%lvl(1)%r, this%phmg_hrchy%lvl(2)%r, &
                           this%msh%nelv, this%phmg_hrchy%lvl(1)%Xh)

    ! Crs solve
    call this%phmg_hrchy%lvl(1)%gs_h%op(this%phmg_hrchy%lvl(1)%r, &
         this%phmg_hrchy%lvl(1)%dm_Xh%size(), GS_OP_ADD)
    
    call bc_list_apply_scalar(this%phmg_hrchy%lvl(1)%bclst, &
                              this%phmg_hrchy%lvl(1)%r, &
    ...
    call this%intrp(1)%map(this%phmg_hrchy%lvl(2)%r, &
                           this%phmg_hrchy%lvl(1)%e%x, &
                           this%msh%nelv, this%phmg_hrchy%lvl(2)%Xh)
    
!    call add2(this%phmg_hrchy%lvl(2)%e%x, this%phmg_hrchy%lvl(2)%r, &
!              this%phmg_hrchy%lvl(2)%dm_Xh%size())

    call this%intrp(2)%map(z, this%phmg_hrchy%lvl(2)%r, &
