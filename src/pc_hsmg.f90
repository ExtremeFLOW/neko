...
!    end do
  ...
  subroutine hsmg_solve(this, z, r, n)
    ...
    call col2(r, this%grids(3)%coef%mult, &
                 this%grids(3)%dof%n_dofs)
    !Restrict to middle level
    call tnsr1_3d(r,this%grids(2)%Xh%lx,this%grids(3)%Xh%lx,&
                  this%jht(1,2),this%jh(1,2), this%jh(1,2), &
                  this%msh%nelv)
    call gs_op_vector(this%grids(2)%gs_h, r, &
                      this%grids(2)%dof%n_dofs, GS_OP_ADD)
    !OVERLAPPING Schwarz exchange and solve
    call this%grids(2)%schwarz%compute(this%grids(2)%e%x,r)  
                                                     !         T
