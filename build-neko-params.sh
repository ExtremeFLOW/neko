#!/bin/bash
# Build script for neko-params
# Compiles the neko-multiphase-parameter-analysis version of Neko
#
# Usage:
#   ./build-neko-params.sh --local            # build for macOS
#   ./build-neko-params.sh --local --clean     # full clean rebuild
#   ./build-neko-params.sh --local --regen     # re-run regen.sh + reconfigure
...
if [ -z "$ENV_FLAG" ]; then
    echo "Usage: ./build-neko-params.sh --local|--cluster [--clean|--regen]"
    exit 1
fi
...
INSTALL_PREFIX="$NEKO_PARAMS_PREFIX"

echo "=== Building neko-params ==="
...
echo ""
echo "=== neko-params installed to $INSTALL_PREFIX ==="
echo "Usage:"
