...
start_of_rea = """****** PARAMETERS *****
   ...
   1.00000E+00     p001 DENSITY
  -1.00000E+05     p002  ==> Re=100k
  0.000000E+00     p003
  0.000000E+00     p004
  0.000000E+00     p005
  0.000000E+00     p006
 ...
 -2.000000E-05     p012 DT
  ...
  0.000000E+00     p063 =8 --> force 8-byte output
  ...
  0.000000E+00     p068 iastep: freq for avg_all (0=iostep)
  0.000000E+00     p069
  0.000000E+00     p070
  0.000000E+00     p071
  0.000000E+00     p072
...
def read_periodic(ifile, periodic_dx):
    ...
    if not periodic_dx:
        keys = list(periodic_face_map.keys())
        vals = iter(periodic_face_map.values())
...
def create_cell_map(dim):
    ...
    for cell, faces in cell_face_map.items():
...
def create_cell_face_map_2D():
    ...
    # Generate face_map
    for cell, nds in cell_map.items():
...
def create_cell_face_map_3D():    
    ...
    for cell, faces in cell_face_map.items():
        ...
        remaining_faces = list(face_dict_numbering.keys())
        ...
        for face in faces: # Loop over remaining faces to be placed
            ...
            common_nodes = find([x in nodes1 for x in nodes2])
            new_nodes = find([x not in cm for x in nodes2])
...
def create_periodic_cell_face_map():
    ...
    of cell 10."""
    for f0, f1 in periodic_face_map.items():
...
def read_curved_sides(curves):
    ...
    for curve_zone, curve in curves.items():
        cell_face_m = []
        curve['x'] = []
        spline = []
        ...
        for nd, facel in boundary_nodes_face_map[curve_zone].items():
            if len(facel) > 1:
    ...
    for face in face_node_map:
        if face in curved_faces[curve_zone]:
            ...
            if fl is not None and fr is not None:
                common_node_left = [x in face_list[face][1] for x in face_list[fl][1]]            
                node0 = face_list[fl][1][common_node_left.index(False)]
                node1 = face_list[fl][1][common_node_left.index(True)]
                common_node_right = [x in face_list[face][1] for x in face_list[fr][1]]            
...
IO_FLD      = N_STEP
    ...
    semtex_bnds = {}
    for key, val in boundary_val_map.items():
        if not val == 'E':
            if val in semtex_bnds:
                ...
                semtex_bnds[val] = [key]
    N = 0
    for key, val in semtex_bnds.items():
        if key == 'P':
            N += len(val)/2
        else:
            N += len(val)        
...
def write_fenics_file(dim, ofilename):
    ...
    for zone, faces in boundary_faces.items():
        for face in faces:
            ...
            for jj, ff in enumerate(facets(dolfin_cell)):
                ...
                if all([x in nodes_of_face for x in facet_nodes]):
                    local_index = jj
                    break
