module global_params
  ...
  real(kind=rp) :: fst_ti	= 3.1d-2! turbulence intensity
  real(kind=rp) :: fst_il = 3.2d-2 / 0.2d0 ! integral length scale		

  integer, parameter :: nshells = 80! No of spherical shells
  integer, parameter :: Npmax = 20  ! Npmax  -  Number of points in a shell
  real(kind=rp), parameter :: kstart = 43.77d0 * 0.2d0! smallest wavenumber
  real(kind=rp), parameter :: kend = 1660d0 * 0.2d0 ! largest wavenumber
