...
#SBATCH -t 08:00:00
#SBATCH -N 4
#SBATCH -J channel_l3_sigma0
#SBATCH -p main
#SBATCH -o /cfs/klemming/projects/supr/kthmech/eriksie/logs/%J_channel_l3_sigma0.log

# L3 two-phase channel: σ=0 CDI diagnostic, 192×32×64 mesh, t=20→25
#
# ε=0.03, Δxz=0.0654, 4ε/Δ=1.83, ε/Δ_GLL=3.2 (constant ratio ε/Δxz=0.45)
# R/2ε=6.67 — sharpest drop in the convergence series
# τ_CDI = ε²/0.075 = 0.012 TU
#
# Purpose: κ_rms convergence point at L3 resolution. Compare with L1 (ε=0.053)
# and L2 (ε=0.04) results. Full 5 TU run for statistics.
#
# Prerequisites:
...
#   sbatch $KTHMECH_PROJECT/scripts/job_channel_l3_sigma0.sh
...
RUN_NAME="channel_l3_sigma0"
RUN_DIR=$SCRATCH_DIR/$RUN_NAME

mkdir -p "$RUN_DIR"
cd "$RUN_DIR"

cp "$SRC/cases/192x32x64/turb_channel_two_phase_l3_sigma0.case" .
...
srun -u -n 512 ./neko turb_channel_two_phase_l3_sigma0.case
