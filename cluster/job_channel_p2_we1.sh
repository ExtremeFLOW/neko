...
#SBATCH -t 06:00:00
#SBATCH -N 1
#SBATCH -J channel_p2_we1
#SBATCH -p main
#SBATCH -o /cfs/klemming/projects/supr/kthmech/eriksie/logs/%J_channel_p2_we1.log

# Phase 2 two-phase channel: We=1 (σ=0.4), 108×18×36 mesh, t=20→25
#
# We=1: σ=0.4, Δt_cap≈0.025, Δt/Δt_cap≈0.16 (still within capillary limit but tighter)
# Target case for turbulent drop dynamics at physically relevant Weber number.
#
# Prerequisites:
#   1. neko_two_phase built: sbatch build_neko_two_phase.sh
#   2. channel_p2_single_phase spin-up completed (fluid00004.chkp at t=20)
#   3. p2_sigma0 and p2_we10 completed or at least running
#
# Submit from Dardel login node:
#   sbatch $KTHMECH_PROJECT/scripts/job_channel_p2_we1.sh
...
RUN_NAME="channel_p2_we1"
RUN_DIR=$SCRATCH_DIR/$RUN_NAME

mkdir -p "$RUN_DIR"
cd "$RUN_DIR"

cp "$SRC/cases/108x18x36/turb_channel_two_phase_p2_we1.case" .
...
srun -u -n 128 ./neko turb_channel_two_phase_p2_we1.case
