...
#SBATCH -t 06:00:00
#SBATCH -N 1
#SBATCH -J channel_p2_we10
#SBATCH -p main
#SBATCH -o /cfs/klemming/projects/supr/kthmech/eriksie/logs/%J_channel_p2_we10.log

# Phase 2 two-phase channel: We=10 (σ=0.04), 108×18×36 mesh, t=20→25
#
# We=10: σ=0.04, Δt_cap≈0.079, Δt/Δt_cap≈0.051 (well within capillary limit)
# Phase 1 baseline blew up at t≈20.44 (0.44 TU). Finer mesh (3.1 elements/interface
# vs 1.8) may delay or prevent blow-up.
#
# Prerequisites:
#   1. neko_two_phase built: sbatch build_neko_two_phase.sh
#   2. channel_p2_single_phase spin-up completed (fluid00004.chkp at t=20)
#   3. p2_sigma0 completed (confirm κ_rms level before running σ>0 cases)
#
# Submit from Dardel login node:
#   sbatch $KTHMECH_PROJECT/scripts/job_channel_p2_we10.sh
...
RUN_NAME="channel_p2_we10"
RUN_DIR=$SCRATCH_DIR/$RUN_NAME

mkdir -p "$RUN_DIR"
cd "$RUN_DIR"

cp "$SRC/cases/108x18x36/turb_channel_two_phase_p2_we10.case" .
...
srun -u -n 128 ./neko turb_channel_two_phase_p2_we10.case
