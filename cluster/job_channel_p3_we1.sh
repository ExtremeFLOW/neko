...
#SBATCH -t 06:00:00
#SBATCH -N 2
#SBATCH -J channel_p3_we1
#SBATCH -p main
#SBATCH -o /cfs/klemming/projects/supr/kthmech/eriksie/logs/%J_channel_p3_we1.log

# Phase 3 two-phase channel: We=1 (σ=0.4), 144×18×48 mesh, t=20→25
#
# We=1: σ=0.4, Δt_cap≈0.026 (at Δx=0.0873), Δt/Δt_cap≈0.15
...
#   3. p3_sigma0 and p3_we10 completed or at least running
#
# Submit from Dardel login node:
#   sbatch $KTHMECH_PROJECT/scripts/job_channel_p3_we1.sh
...
RUN_NAME="channel_p3_we1"
RUN_DIR=$SCRATCH_DIR/$RUN_NAME

mkdir -p "$RUN_DIR"
cd "$RUN_DIR"

cp "$SRC/cases/144x24x48/turb_channel_two_phase_p3_we1.case" .
...
srun -u -n 256 ./neko turb_channel_two_phase_p3_we1.case
