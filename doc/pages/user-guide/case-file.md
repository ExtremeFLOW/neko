...
| `initial_condition.interpolate`                    | Whether to interpolate the velocity and pressure fields from the field file onto the current mesh.| `true` or `false`                                           | `false`       |
| `initial_condition.mesh_file_name`                 | If interpolation is enabled, the name of the field file that contains the mesh coordinates.       | Strings ending with `f*****`                                | `file_name`   |
| `initial_condition.interpolation.tolerance`        | Tolerance for the point search.                                                                   | Positive real.                                              | `NEKO_EPS*1e3`|
| `initial_condition.interpolation.padding`          | Padding for the point search.                                                                     | Positive real.                                              | `1e-2`        |
