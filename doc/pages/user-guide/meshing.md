...
Since gmsh is popular, a lot of meshing software supports it. There is a caveat though: thre have been several versions of the `.msh` and `gmsh2nek` supports only a particular flavour. Moreover, it expects the order of the mesh to be 2, whereas most software will produce a linear mesh. The typical meshing pipeline is therefore the following:
...
example, from `.geo` file. This is done in the `turb_pipe` example. The README
