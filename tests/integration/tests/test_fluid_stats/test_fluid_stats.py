...
def test_fluid_stats(launcher_script, request, log_file, tmp_path):
    ...
    The test runs 3 fluid_stats simcomps one with xy averaging and one with just
    ...
    csv = np.genfromtxt(join("tests", "test_fluid_stats", "fluid_stats0.csv"),
                        delimiter=",")[12, 2:]
...
#    pynekread(join("tests", "test_fluid_stats", "stats2d0.f00001"), comm,
...
#    mesh = Mesh(comm, create_connectivity=True)
#    fld = FieldRegistry(comm)
#    pynekread(join("tests", "test_fluid_stats", "stats3d0.f00001"), comm,
#              data_dtype=np.single, msh=mesh, fld=fld)
#
#    # Ordering of keys corresponds to 'correct' array above
