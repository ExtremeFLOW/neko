...
```bash
cd path/to/neko/examples/tgv
/path/to/neko_install/bin/makeneko tgv.f90
mpirun -np 4 ./neko tgv.case
```
If there is no .f90 (user) file in the example, the standard executable `neko` can also be used, for example:
```bash
cd examples/hemi
mpirun -np 4 /path/to/neko_install/bin/neko hemi.case
```
This example only uses built-in functions and does not need a compiled user file. Whether you will need a user file or not depends on what functionality you want and this is also documented in the documentation.
...
* Morse, N., Jansson, N., Bode, M., Lingwood, R. J., Alfredsson, P. H., Mihaescu, M., Örlü, R., Schlatter, P., 2026. *Direct numerical simulation of the rotating disk turbulent boundary layer*. Proceedings of the 14th International Symposium on Turbulence and Shear Flow Phenomena (TSFP14). [http://www.tsfp-conference.org/proceedings/2026/125.pdf](http://www.tsfp-conference.org/proceedings/2026/125.pdf)
* Toosi, S., Massaro, D., Schlatter, P., 2026. *Outer-layer similarity and inner-outer interactions in pipes with regular sinusoidal rough walls*, Proceedings of the 14th International Symposium on Turbulence and Shear Flow Phenomena (TSFP14). [http://www.tsfp-conference.org/proceedings/2026/45.pdf](http://www.tsfp-conference.org/proceedings/2026/45.pdf)
